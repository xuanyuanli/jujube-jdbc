...
public class HandlerTest {
    ...
    public void testHandler1() {
        ...
        assertThat(spec.getFilterSql())
                .isEqualTo("(`age`= ? and `name` like ? and `type` between ? and ? and `source` in(1,2,3) and `title` is null and `sub_title` is not null and `mobile` <> ?)");
